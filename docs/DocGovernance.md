...
- [x] **Onboarding ADR 指针纠偏**：A.4/A.5/A.7 三处原指向被掏空归档 `项目演变历史-0.9.1.md` 的 ADR 指针，改为 → `RootDoc-ADR.md`（L0 索引/速查）+ 根 `CHANGELOG.md`（完整记录）+ `项目待办事项-优先级重排.md`（待评估）。消除"活文档引用只读归档"的红线违反。
- [x] **孤儿文档登记**：`项目待办事项-优先级重排.md`（2026-08-18 起，原 `项目待办事项.md` 并入）、`FeatureModuleVisibility-Plan.md` 补登 `docs/README.md` 根级表。（FeatureModuleVisibility-Plan.md 已于 2026-08-09 并入 `项目演变历史-0.9.9.md`，2026-08-17 随全量合并抽离至 `docs/plans/功能可见性管理方案.md`，2026-08-18 随 plans/ 归档删除）
- [x] **历史引用边界强化**（§2 矩阵 + §3 反模式 #1 改写）：明确活文档**仅可引用变更记录主索引根 `CHANGELOG.md`**，禁止引用已删除的 `docs/archive/` 下 `项目演变历史-0.9.x` 等分卷（2026-08-17 并入根 CHANGELOG.md）；ADR 完整记录归属主索引、待评估项归 `项目待办事项-优先级重排.md`。
