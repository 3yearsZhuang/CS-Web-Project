...
1. **历史文档不得承载活内容（仅主索引可被活文档引用）**：`项目演变历史-0.9.x.md` 等分卷为**只读归档**，活文档**不得链接分卷、不得从中提取活内容**；唯一可被活文档引用的历史入口是**主索引 `项目演变历史.md`**（完整 ADR 记录 / R 表存放处），且其活内容已按 §2 矩阵归属到各 SSOT（如 ADR 索引/速查→`RootDoc-ADR.md`、待评估项→`项目待办事项.md`）。
