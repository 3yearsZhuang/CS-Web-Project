...
> 备份/恢复统一用 `CS-Web-Backend/tools/scripts/db/backup_db.sh`；导出到桌面用根级 `scripts/ops/export_db_to_desktop.sh`。

### 1. 导出（开发环境）

```bash
# 根级脚本：导出开发库到 ~/Desktop（默认）
./scripts/ops/export_db_to_desktop.sh
# 或指定目录
./scripts/ops/export_db_to_desktop.sh /path/to/out

# 也可用后端脚本直接备份（容器内 db 服务名可直连时）
./CS-Web-Backend/tools/scripts/db/backup_db.sh /path/to/backup
```
...
./CS-Web-Backend/tools/scripts/db/backup_db.sh --restore /opt/cs-backup/domefff_xxx.sql.gz
...
| Jenkinsfile | **等价备用验证（自托管）** | 由 Jenkins 控制器按需触发 | （见备注） | 当前**不含部署 / 发布阶段**，仅为验证对等体；ER-09 文档死链审计已从 Jenkinsfile 移除，改由根仓 `ci.yml` 全权负责（消除子模块对根仓脚本 `scripts/check/check_dead_links.py` 的反向依赖，并避免子模块单独检出时 `../scripts` 缺失导致该步空跑/失败） |

> 两套都只做「验证」，均不含部署 / 发布阶段（部署见本文 §四 容器化部署）。
> ER-09 文档死链审计由根仓 `.github/workflows/ci.yml` 的 `Docs dead links (ER-09)` 步骤（`scripts/check/check_dead_links.py --base . --docs docs`）单一承担；Jenkinsfile 不再反向引用根仓脚本。
