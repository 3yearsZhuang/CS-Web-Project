...
- **「用量与设置」独立详情页**：工作台小组件与全量 Agent 页不再内嵌展开用量/设置面板——头部「用量与设置」按钮改为跳转新建独立页 `/tools/auxilio/settings`（LlmUsageStats 完整视图 + 返回链接），小组件保持纯轻聊、全量页保持对话主区；workbench README 同步。ts-check 10 基线零新增、页面渲染验证通过。
