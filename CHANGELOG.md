...
- **「用量与设置」独立详情页**：用量/设置从内嵌面板收敛为独立页 `/tools/auxilio/settings`（LlmUsageStats 完整视图 + 返回链接）；**「用量与设置」按钮仅存在于全量页 /tools/auxilio 顶栏**（跳转设置页），工作台小组件为纯轻聊、无用量入口；workbench README 同步。ts-check 10 基线零新增、页面渲染验证通过。
