...
> 规划类长文（工具页→工作台改造方案、功能模块可见性方案）已于 2026-08-18 归档处置（C-20）：落地结论见各版本 `Added`/`Changed`，未完成残余项见 `docs/项目待办v2.md`（C-21 起为待办 SSOT）。
...
- **文档整合·前端 UI 收口历史归档（2026-08-20，P1）**：原 `FrontDoc-UIButton.md` 的按钮/控件统一审计与 Batch 迁移史归档至此，文档已删除；统一用法（8 变体、选型矩阵、禁止项、输入框/徽章/Tab/分页/Modal/z-index、质量门禁）现为 `FrontDoc-UID.md` §5.2 + §16 唯一权威。历史回溯：**2026-08-17 全站 UI 收口完成**——散落描边小按钮清零（39 文件）；`<Button>` 扩展 `outline-danger`/`ghost`/`xs`/`active` + `primary-outline`/`amber`/`filled` 变体；Pagination 扩展 `window/ellipsis/all`；`INPUT_CLASS`、`<Badge>`、`.tab-chip`/`.tab-underline`、`ModalShell`、z-index CSS 变量化；`btn-danger-sm` 尺寸契约修复（实色变体 `border:1px solid transparent` 盒模型对齐）。

- **重复实现治理·波次 D（P3 清理收尾，2026-08-20）**：D1 删除全站搜索聚合后的 18 行孤儿 i18n 词条（community/communityAdmin/events/admin userList 的 searchPlaceholder 系列，common ns 保留）；D2 后端清理——`refresh_token_repo._now()` 冗余别名删除直引 `now_utc`（B16）、3 处手写 `isoformat() if x else None`（auxilio/contribution/audit）收敛到 `core/timezone.iso_or_none`（B17）。验证：ts-check 10 基线零新增、vitest 233/233、app.main import OK、契约零漂移。**至此四波次（A/B/C/D）全部闭环，治理计划完成。**

- **重复实现治理·波次 C（P2 样板收敛，2026-08-20）**：C1 前端——#12 joinStatusMeta 收敛双套状态映射、#14 日期统一 formatDate、#23 events not-found 用 BackLink、#20 5 处手写 Spinner→Spinner 原语、#21 exam 骨架屏→Skeleton 原语、#27 成功提示条样板→新 `useTransientMessage`、#28 topics-manager 排序条→InlineTabs、#24/#25 15 处手写空态+6 处错误态→EmptyState（新增 `tone="error"` 变体）、#16 3 个 use-*-manager 收敛到新 `useAdminCollection` 工厂、#19 admin-feature-visibility-panel 手写弹窗→ModalShell。C2 后端——B10/B11 community /follows 与 admin /topics /reports 分页对齐 PaginationParams+PaginatedResponse（契约刷新 181 路由）、B12 `get_user_with_roles` 跨 repo 收敛（user_repo 深度参数 + rbac_repo 委托）。验证：ts-check 10 基线零新增、vitest 233/233、app.main import OK、契约零漂移。保留/暂缓记录：#15 公告双套面板合并（入口迁移评估）、#17 use-community-feed（外部并发中）、#18 about Promise.all 并行形态、#22 TechTagSelector 折叠 vs 平铺、#26 主题卡片（FeedItem/CommunityPost 数据形态 + 外部并发）、#13 已收敛、B13 不同表同构、B14/B15 低收益机械替换留后续。

- **重复实现治理·波次 B2（后端复制收敛，2026-08-20）**：B3 `_to_event_out`（events/admin_events 逐字重复）→ `schemas/event.event_to_out`；B4 管理员出参 → `user_service.to_admin_out`；B5 配置时区当日零点（workbench/auxilio_tool_repo 相同实现）→ `core/timezone.local_day_start_utc`；B6 FTS 构造三处（community 路由 / community_repo / search_service）→ `core/query_helpers.fts_condition`；B7 JSONB 标签过滤五处（community / event_repo / tools_repo×2 / community_repo）→ `core/query_helpers.jsonb_contains`；B8 search_service 过滤段与 5 处 item 构造 → `_match_text` / `_item`；B9 批量补作者（community_post / community_comment 同骨架）→ `community_utils.load_users_by_ids`。验证：app.main import OK、契约零漂移、JSONB helper DB 实测 OK（FTS 本地缺 chinese 全文检索配置为环境限制，CI/生产有迁移）。
- **重复实现治理·波次 B1（前端类型收敛，2026-08-20）**：11 组重复类型统一 import——Announcement 组（use-announcements / admin-announcements-panel / community use-announcements-manager 统一 `modules/announcements/types`，announcement-banner 纯 UI 壳按公共层零业务依赖原则保留本地类型）、JoinApplication 组（admin-join-panel / join-tab / about 统一 `modules/join/types`，AppStatus→JoinApplicationStatus）、tools 组（use-resources ResourceItem→ResourceWithAuthor、exam 页 ExamItem→Exam、**exam-countdown 校准 endedAt→endTime 修复数据源读取 bug**）、TargetType 语义冲突（report-button / community-actions 改用既有 `LikeTargetType`）、TopicStatus→PostStatus 子集。暂缓记录：tool-types 的 Exam/Task/TaskClaim（snake_case 数据源待确认）、YearGroup（EventItem 双源待统一）。验证：ts-check 10 基线零新增、vitest 233/233。
- **重复实现治理·波次 A（P0 行为不一致，2026-08-20）**：① **资源搜索统一**——`AuxilioToolRepository.search_resources` 升级为标题/描述双 ilike（保留通配符转义、浏览量倒序），全站搜索 `search_service._search_resources` 改透传该 repo 方法（删除 100 条内存过滤），学习助手工具与全站搜索对同一关键词结果一致、不再受 100 条上限；② **默认模型统一**——workbench LlmConfigIn / workbench_service / auxilio_agent / llm_config 4 处 `"gpt-4o-mini"` 硬编码 fallback 全部改为 `settings.LLM_MODEL`（权威配置，改配置全量生效）。验证：两入口结果一致性断言 PASS、LLM_MODEL 环境变量生效断言 PASS、契约零漂移、回归无新增失败（既有 test_resource_flow/test_auxilio 测试桩问题 `db.execute(Select,0,20)` 为 B2 时代遗留，子仓 CI 覆盖）。文档 `BackDoc-01-Arch.md` §六 Skills 同步。

- **学习助手 Agent 预设（融合点 3，吸收 DSH「Agent 预设」）**：`auxilio_agent.py` 新增 `AgentPreset` 声明式注册表（`AGENT_PRESETS`：`general` 通用答疑全量工具 / `exam_sprint` 考试冲刺（3 工具 + temperature 0.3）/ `resource_finder` 资源检索（2 工具 + temperature 0.5））——每预设 = 系统提示词模板（`PRESET_TEMPLATES`，占位符经 `build_system_prompt` 填充、用户字段仍走 ER-19 包裹）+ 工具子集 + 模型参数。`run_chat(preset_id=...)` 显式指定，缺省按用户首条消息关键词启发式匹配（`match_preset`，考试/资源/通用，有序优先），无效 id 视同未指定。`llm_client.stream_chat` 新增可选 `temperature` 透传（OpenAI/Anthropic 双协议，None 不传保持默认）。验证：注册表完整性 / 启发式匹配 / 显式优先 / 模板渲染 / run_chat 工具子集 + temperature 透传全部断言通过；ER-18 回归 1 passed。**前端与 API 契约零改动**（预设解析在服务层）。文档 `BackDoc-01-Arch.md` §六「Agent 预设」+ `BackDoc-03-Conv.md` §14.2 同步。
- **学习助手联网搜索 + 全量 Agent 页（融合点 4 + 前端能力拆分）**：① 后端新增 `web_search` 联网搜索工具（DuckDuckGo 免费接口、无需 API key，`WEB_SEARCH_ENABLED` 可配置关闭，结果经 ER-19 包裹标记不可信）+ `web_research` 预设（联网研究：web_search / search_resources / analyze_learning_profile，temperature 0.4），`general` 预设自动纳入全量工具（现 8 个）；② 契约变更：`POST /api/v1/auxilio/chat` 请求体新增可选 `preset_id`，新增 `GET /api/v1/auxilio/conversations/{id}/events`（Trajectory 回放，融合点 2 消费端）——`contract-baseline` 刷新（179 → 180 路由）零漂移 + 前端 `backend-api.d.ts` 同步；③ 前端：`assistant-chat` 迁移至 `modules/auxilio/ui/` 并支持 `mode`——工作台小组件用 **lite 纯轻聊**（仅提问+回复，无会话列表/无工具卡/无历史加载），llm-widget 同步接入；④ 新建全量 Agent 页 `/tools/auxilio`（`modules/auxilio/ui/agent-page.tsx`：full 对话 + Agent 预设切换器 + Trajectory 回放面板（逐条播放高亮）+ 用量统计），`/tools` 工具区新增「学习助手 Agent」入口；⑤ BFF 新增 `conversations/[id]/events` 透传路由。验证：契约零漂移、ts-check 10 基线零新增、vitest 全绿、本地端到端（preset_id 对话 / events 回放 / 全量页渲染 / /tools 入口）通过。文档 `BackDoc-01-Arch.md` §六同步。
- **用户级功能开关（LLM 设置面板）**：`llm_configs` 新增 `web_search_enabled` / `trajectory_enabled` 两列（Alembic `e5f6a7b8c9d0`，默认 True 兼容旧行为）——前端「用量与设置」面板新增「启用联网搜索」「记录对话轨迹（Trajectory）」两个复选框（用户级持久化）；`web_search` 工具在全局 `WEB_SEARCH_ENABLED` 与用户开关同时为真时才可用；`/auxilio/chat` 在用户关闭轨迹记录时不再写 `chat_events`。BFF `/api/workbench/llm-config` 透传新字段（camelCase ↔ snake_case 转换）。验证：契约零漂移（180 路由）、ts-check 10 基线零新增、vitest 234/234、本地端到端（开关保存/回读、关闭后 chat_events=0 且 web_search 返回「已在设置中关闭」，重新开启恢复）通过。文档 `BackDoc-01-Arch.md` §9.2 / 迁移 head 同步。
- **「用量与设置」独立详情页 + 全量页规范布局**：用量/设置从内嵌面板收敛为独立页 `/tools/auxilio/settings`（LlmUsageStats 完整视图 + 返回链接）；「用量与设置」按钮仅存在于全量页 `/tools/auxilio`（跳转设置页），工作台小组件为纯轻聊、无用量入口；**全量页与设置页重构为工具区页面规范布局**（折叠 Hero CollapsingHero + pixel-page + section 导航，参照 /tools/exam），修复顶栏控件错位；删除并入 page.tsx 的 `agent-page.tsx`。i18n 补 agentEn/agentChatSection/agentBackTools/settingsBack。ts-check auxilio 相关零错误、页面渲染（pixel-page + section）验证通过。
- **统一返回按钮为公共组件 BackLink + 全二级页补全返回**：新增 `components/layout/back-link.tsx`（meta-mono 样式，以 /tools/auxilio/settings 设计为准；`arrow` 开关防词条自带箭头重复），经 `@/components` 导出；替换 7 处既有 sidebarBottom 返回（exam / task / resource / events/[id] / auxilio / auxilio/settings）+ 社区详情 TopicHero + 考试答题页内返回；**补全缺失返回的二级页**：tools/dev-center、community/new、community/drafts、community/series/[id]、community/tags/[tag]、users/[id]（均为 Hero sidebarBottom）。i18n：tools.ts 清理 agentBackTools/settingsBack 的 `←` 前缀、community.ts communityNew 补 backToAll、user.ts userPublic 补 backToCommunity（zh+en+类型）。验证：ts-check 10 基线零新增、页面渲染抽查（dev-center / community/new / drafts 返回链接）通过。
- **全站搜索聚合到顶栏（2026-08-20）**：① 后端新增 `GET /api/v1/search?q=&scope=&limit=` 聚合端点（`app/api/v1/search.py` + `services/search_service.py`）——scope 支持 `all|events|community|tools|announcements|users`，events/community posts 透传既有 `search` 参数、users 复用 `search_vector` 全文检索、announcements/resources 数据量小走服务内 Python 过滤，统一输出 `SearchResultItem`（type/id/title/subtitle/url）；② BFF `src/app/api/search/route.ts` 薄转发；③ 顶栏 `GlobalSearch`（`modules/search/` 域，按三端模块命名规范落位）——路由感知范围（hero `/` 全站、`/events`/`/community`/`/tools`/`/users` 单模块）、防抖 300ms 即时下拉分组结果、scope 徽标、键盘导航，navbar 桌面右侧 + 移动抽屉顶部；④ 下拉底部「查看全部结果 →」跳 `/search` 完整结果页（`app/search/page.tsx` + `modules/search/ui/search-results.tsx`，每类 10 条）；⑤ **取消各模块单独搜索（全部移除）**：/events `EventFilterBar` 去搜索框（仅留状态筛选）、/community 搜索条及 `use-community-feed` 搜索逻辑（URL `q` 参数/handleSearchSubmit/Clear/`searchInputRef`）移除、管理面板（topics-manager/users-manager/admin-users-panel/user-list-view/use-user-list/use-admin-users）搜索全移除，`community-search` 可见性组件不再渲染（registry 配置保留）。i18n nav 命名空间新增搜索文案（zh/en）。验证：契约零漂移（181 路由）、ts-check 10 基线零新增、vitest 233/233、next build 通过。
...
- **目标 2 完整收口 + 规范文档建立（2026-08-19）**：① app/ 页面目录散落业务组件全部归位（20 文件：12 组件 + 4 hooks + 4 tests → `modules/{auth,notifications,tools,community}/ui/` 与 `ui/hooks/`，app/ 仅剩 Next.js 页面壳）；② 公告横幅跨层依赖根治（A2：`useAnnouncements` 归位 announcements 域 + banner 改纯 UI 壳 + 新增 `announcement-banner-client` 数据容器，公共层零业务依赖）；③ 建立 `docs/RootDoc-ModuleMap.md`（三端模块 ↔ 契约资源映射 SSOT，命名规范/门禁/违约点红线/变更记录；2026-08-20 由 `模块命名映射表.md` 改名）并登记文档地图，`RootDoc-EngConv.md` 命名小节改索引、后端 `AGENTS.md`/前端 `modules/README`/移动端 `tools/docs/README` 纠偏同步（包化例外与资源域结构）。
...
- **工作台 Schema 管理卡（表单+实时预览+已建列表三合一）**：`SchemaWidgetRenderer` 由「渲染已配置卡」改造为独立管理卡——表单（受控 `SchemaCardForm`，状态提升）+ 顶部实时预览（`buildConfigFromDraft` 即时构建，合法即渲染）+ 已建卡列表；布局设置面板不再内嵌表单；schema-widget 默认尺寸 2x2→2x3；i18n 补 `schemaPreview`/`schemaPreviewLive`/`schemaPreviewHint`。
- **工作台排序模式统一尺寸拖拽（根治拖拽畸变）**：变尺寸积木 + `grid-auto-flow:dense` 与 dnd-kit 排序策略存在结构性冲突（`rectSorting`/`rectSwapping` 在非均匀网格均会畸变），改为「编辑模式」——正常态纯展示（无拖拽手柄，根治手柄与文字重叠）；进入「排序模式」（布局面板按钮）后所有卡渲染为统一尺寸缩略卡（`SortThumbnail`，`aspect-square`），同尺寸下 `rectSortingStrategy` 让位稳定零畸变，`DragOverlay` 浮层尺寸精确一致；退出后按新顺序恢复积木布局。保留布局面板「顺序调整」列表（上移/下移）为精确微调通道。i18n 补 `sortingEnter`/`sortingExit`/`orderHint`，`dragHint` 文案更新。
...
- **学习助手工具声明式注册表（融合点 1，吸收 DSH「一切皆插件」思想）**：`auxilio_agent.py` 的工具从硬编码 `TOOL_SCHEMAS` + `execute_tool` if/elif 链重构为声明式注册表——`ToolSpec`（name / description / parameters / handler / exposed）+ `TOOL_REGISTRY`，`TOOL_SCHEMAS` 由注册表推导（仅 `exposed=True` 暴露给模型）；handler 约定 `async (db, user, args) -> str`。**行为不变**（7 个工具 schema 及顺序、未知工具 error、参数透传均与重构前一致）；隐藏工具 `get_api_usage_stats`（ER-18）改为显式 `exposed=False`（模型不可见、`execute_tool` 仍可调用，权限边界测试通过）。验证：注册表等价性断言脚本 + ER-18 pytest 用例通过；文档 `BackDoc-03-Conv.md` §14.2 同步。为后续融合点（Trajectory 日志 / Agent 预设）打地基。
...
- **1.0.0 发布门槛#3 定义补列并闭环（SSOT 同步）**：门槛#3 原清单未枚举，已于 2026-08-19 补列定义为「端到端验收闭环（E2E 发布前必过）」——真实 docker-compose 全栈部署形态下核心用户旅程 E2E 全绿，作为 1.0.0 发布前必过（非仅 nightly）；**已于 2026-08-19 闭环**：根仓 `e2e.yml` 加 `push:main` 触发，合并到 `main` 即跑全栈 E2E，保证默认分支「可发布绿」（保留 nightly + 手动），对应 P0-4。见 `docs/项目待办v2.md`。
...
- **文档冲突修正（版本事实 0.9.8 → 0.9.9，2026-08-17）**：`文档冲突审查` 多份页面版本号对齐 0.9.9——待办 F-11 四源版本（F-1）、`RootDoc-Deploy`（F-2）、根 README（F-3）、`api-reference`/`README` 0.9.8 冻结契约标注（F-4）、Onboarding 进度（F-5/F-10）、多份后端文档页眉刷新 0.9.9（F-6）随上述 C-* 动作一并修正；社区重构文档状态（F-8）见 C-12。剩余 F-9（alembic heads 实测复核）转入待办 `项目待办v2.md`。
- **规划长文归档处置（C-20，2026-08-18）**：`docs/plans/` 两份规划（工具页→工作台改造方案、功能模块可见性管理方案）落地结论均已归档于 `[0.9.8]`/`[0.9.9]` Added，无需重复登记；未完成残余项（LeetCode 热力图、每周复盘卡、系统状态角标、浏览器 Notification 可选增强、数据导出、可见性定时上下线）移交 `docs/项目待办事项.md`（2026-08-18 并入 `docs/项目待办v2.md`，见 C-21）；删除 `docs/plans/` 文件夹，同步根文档地图（`docs/README.md`）与 `docs/README.md` 引用。
- **待办 SSOT 合并（C-21，2026-08-18）**：原 `docs/项目待办事项.md`（待办 SSOT）经全量条目对照并入 `docs/项目待办v2.md` 后删除——已迁移条目（P0/P1/P2/P3 主体）按重排结果生效；未迁移条目新建「附录 A」收纳（AR-M3、F-9、LeetCode 热力图、可见性-定时上下线、ER-49/50·51/52/54·57、Phase 4 纵深）；同步修正 D10（已拍板：Wiki/活动评价纳入 1.0、DM/相册延后 P1）与 AR-S2（轻量闭环已落地）结论；重排文件升级为待办唯一权威跟踪（SSOT）；全仓 14 个文档引用点统一改指新文件名。
- **文档治理收口固化（DocGov，2026-08-09 批次，2026-08-19 补登）**：系统性跨仓死链修复（13 处 + 1 提及，根→子仓 `../CS-Web-*`、子仓→根 `../../../docs/`、子仓↔子仓 `../../../CS-Web-*/tools/docs/`）；坏锚点修复 3 处（`BackDoc-Infra` `#六迁移验证`→`#六迁移验证migration_verification`、`RootDoc-MigEval` 全角 slug 归一，共 2+1 处）；ADR 抽离 `RootDoc-ADR.md`（设计决策 SSOT）并解除活文档对归档演变历史的链接（17 处 markdown + 1 处代码注释）；超大文档章节速查导航注入（5 份 600+ 行：`FrontDoc-01-Arch`/`BackDoc-01-Arch`/`BackDoc-Infra`/`FrontDoc-UID`/`FrontDoc-Ops`），幂等脚本 GitHub-slugify 生成；版本号四源同步约定（pyproject/`__version__`/package.json/uv.lock，改版本须四处同改）+ 术语一致约定（repo 层「子仓库(submodule)」、代码内「子模块/模块」）固化入 `docs/README.md` §3 反模式；`monorepo-doc-audit` Skill 锚点腐化审计脚本固化（全角标点 slug 口径）。复跑双审计确认 0 死链 + 0 坏锚点。
...
- 文档重叠核查：`CLAUDE.md`/`AGENTS.md`、`MigEval`/`MigV`、`FrontDoc-Ops`/`BackDoc-03-Conv`、`RootDoc-EngConv` 分工清晰不重叠，无需合并；前端 `tools/docs/` 补 `README.md` 索引。
...
| `M1/M2…` `P1/P2/P3`           | 里程碑 / 文档合并优先级                        | 项目待办v2.md       |
| `R1~R20`                      | 风险登记表（Risk）                          | [0.9.1]                |
| `L1~L10`                      | 法律/合规类待办条目                           | 项目待办v2.md       |
| `F1~F3`                       | 前端设计评审项                              | [0.9.1]                |
| `D1~D4`                       | 前后端分离迁移决策                            | [0.9.1]                |
| `B1~B3`                       | 1.0.0 发布阻塞项（Blocker）                 | [0.9.7]                |
| `G1~G4`                       | 1.0.0 GA 缺口                          | [0.9.7]                |
| `CodeGov-*` `DocGov-*` `AL-*` | 代码/文档治理项、BFF 安全边界                    | [0.9.9]                |
| `C-1~C-21`                    | 0.9.9 时代清理/重构交付项（C-1~C-14、C-20、C-21 已执行归档，C-15~C-19 架构重构待办） | [0.9.9] / 项目待办v2.md |
| `F-1~F-9`                     | 文档冲突审查项（F-1~F-8 已随清理修正，F-9 待办）      | [0.9.9] / 项目待办v2.md |
| `N-01~N-14`                   | 规范化建议项（已落地部分见 C-*，残余 N-01/N-12/N-13/F-9 待办） | 项目待办v2.md |

> 本文件（根 CHANGELOG.md）为全项目变更记录单一事实源；活文档（L0/L1/L2）引用历史请以本文件为准。待办跟踪唯一权威见 `docs/项目待办v2.md`。
