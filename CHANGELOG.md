...
### Changed
- 社区服务收官（ER-15 Phase 4）：新建 `app/services/community_feed.py`（FeedService，关注 CRUD/列表/计数 + _format_follow_users + _notify_follow）、`community_category.py`（CategoryService 分类 5 方法）、`community_report.py`（ReportService 举报 3 方法）、`community_series.py`（SeriesService 系列 3 方法）；`api/v1/community.py` 与 `admin_community.py` 剩余 15 处注入切换（categories→Category、follows/follow-status/follow-counts→Feed、reports→Report、series→Series）；**删除 `app/services/community_service.py`**（方法集差集对比 20 业务方法全部落位、零遗漏；`hash_ip_for_view` 函数内 import 改指 `community_utils`）。**ER-15 God Module 全量收口，AL-2 五服务（Post/Comment/Reaction+Favorite/Feed/Notification）闭环**；API 契约不变，全量 432 passed 无回归 + OpenAPI 契约零漂移。
- 社区服务再拆（ER-15 Phase 3）：新建 `app/services/community_post.py`（PostService，帖子 15 方法 + 收藏列表 + _enrich_posts）、`app/services/community_comment.py`（CommentService，评论 8 方法 + list_by_author）、`app/services/community_notifications.py`（notify_mentions / notify_comment_reply 共享模块，子服务不互持实例）；`api/v1/community.py` 与 `api/v1/admin_community.py` 帖子/评论端点改注入新服务，分类/关注/举报/系列仍走 `CommunityService`（为 Phase 4 FeedService 铺路）；`community_service.py` 从 50 方法精确删 29 方法瘦身至 21 方法（方法集差集对比零意外删除，Phase 2 误删教训落地）；`_is_admin` 改收 AsyncSession；集成测试按域适配。API 契约不变，全量 432 passed 无回归。
...
- 架构快赢（ER-55/ER-56 + 社区常量统一）：`config.py` 518 行单类按域拆为 `app/core/config_parts/` 6 个子模型（database/security/web/rate_limit/ops/business），主 `Settings` 多继承聚合保持 `settings.X` 扁平访问（ER-55）；`app/core/events.py` 无事件循环路径复用模块级应急 loop，消除启动期反复 `asyncio.run` 建/关（ER-56）；`COMMUNITY_LIMITS`/`MENTION_PATTERN` 统一迁入 `app/core/constants.py` 单一来源——消除 service 与 schemas 双源漂移（TITLE_MAX 120 vs 100、CONTENT_MAX 20000 vs 10000）并剔除 service 侧未使用键。
- 架构中期（ER-15 Phase 0 + ErrorCode 演进首批）：`community_service` 纯函数/映射提取至 `app/services/community_utils.py`（hash_ip_for_view / scan_mentions / generate_slug / to_author_summary，re-export 兼容），完成 ER-15 重构史诗 Phase 0 基线加固（ER-15 Phase 1+ 仍 P2/P3）；错误码业务自治首批落地——用户域错误码迁至 `app/services/user/errors.py`（UserErrorCode），全局 `ErrorCode.User` re-export，6 处调用点迁移，验证「业务模块自治错误码」预留机制可行。
- 架构中期（ER-15 Phase 1 + Phase 2 + ErrorCode 全域）：社区通知事件化——`community_service` 5 个 `_notify_*` 通知改发领域事件（`notification_events.py` 新增 5 订阅者独立 session 落库），移除 NotificationService 依赖（Phase 1）；互动服务拆分——新建 `app/services/community_interaction.py`（ReactionService + FavoriteService）迁移 7 个互动方法，api 互动端点改注入新服务（Phase 2）；错误码业务自治全域——Auth/Community/Event/Authorization 四域错误码迁至各 `app/services/*/errors.py`（User 域首轮已迁），共 5 域 51 处调用点全部就位，`ErrorCode.<域>.X` 访问形式不变。
